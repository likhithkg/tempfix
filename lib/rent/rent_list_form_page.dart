// lib/rent/rent_list_form_page.dart
import 'dart:io';
...
class _RentListFormPageState extends State<RentListFormPage> {
  ...
  final _locationController = TextEditingController(); // user-friendly location
  ...
  final _supabase = SupabaseStorageService();
  ...
  final String _locationIQApiKey = "pk.56ccd9d8fb2cd5f3e9d7a656e3b52566"; // 🔑 your API key
  ...
  Future<void> _uploadToSupabase() async {
    ...
    final url = await _supabase.uploadImage(_picked!);
    if (url != null) {
                  ...
                  TextField(
                    ...
                    onChanged: (val) async {
                      ...
                      if (res.statusCode == 200) {
                        setState(() => results = json.decode(res.body));
                      }
                    },
                  ),
  ...
  Future<void> _submit() async {
    ...
    if (_type == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Select a machine type')),
      );
      return;
    }
    if (_lat == null || _lng == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Set location first')),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isEdit ? 'Edit Machine' : 'List New Machine'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'Search & set location',
              ),
              validator: (v) =>
                  (v == null || v.trim().isEmpty) ? 'Select location' : null,
            ),
            const SizedBox(height: 16),
            ...
            FilledButton.icon(
              ...
              label: Text(isEdit ? 'Update Listing' : 'Submit Listing'),
            ),
          ],
        ),
      ),
